...
# Create a dataframe with local schema
...
trainTestArray: List[DataFrame] = dataDF.randomSplit([0.8, 0.2], 1)
...
tpipeline = Pipeline().setStages([vectorAssembler])
fitpipeline = tpipeline.fit(trainingDF)
assembledTrainingDF: DataFrame = fitpipeline.transform(trainingDF)
assembledTrainingDF.show(6)
# need to assemble test data as well
testpipeline = tpipeline.fit(testDF)
assembledTestingDF = testpipeline.transform(testDF)
...
sample_rdd = [Row(-1.5),Row(2.5), Row(8.5), Row(31.4)]
sample_rdd: RDD[Row] = spark.sparkContext.parallelize(sample_rdd)
sample_schema = StructType([
    StructField("X", DoubleType(), nullable=True)])
newDF = spark.createDataFrame(sample_rdd,sample_schema)
XAssembler: VectorAssembler = VectorAssembler().setInputCols(["X"]).setOutputCol("features")
xpipeline = Pipeline().setStages([XAssembler])
testpipeline = xpipeline.fit(newDF)
assemblednewDF = testpipeline.transform(newDF)
# testing the trained ML model on new data X
XtestPredictions: DataFrame = lrModel.transform(assemblednewDF)
XtestPredictions.show()
...
numberOfWords: int = articlesRdd.flatMap(lambda line: line.split(" "))
numberOfWords = numberOfWords.count()
print(f"numberOfWords: {numberOfWords}")

numberOfCharacters: int = articlesRdd.map(lambda line: len(line.replace(" ", "")))\
...
sixletterword = articlesRdd.filter(lambda word: len(word) == 6)
words6Count: int = sixletterword.count()
print(f"6-letter words: {words6Count}")

commondict = sixletterword.map(lambda word: (word, 1)).reduceByKey(lambda a, b: a + b)
commonWord = commondict.max(key=lambda x: x[1])
commonWord: str = commonWord[0]
commonWordCount: int = commonWord[1]
