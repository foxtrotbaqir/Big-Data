...
weatherDataFrame: DataFrame = spark.read.option("inferSchema", "true").option("header","true").csv("abfss://shared@tunics320f2023gen2.dfs.core.windows.net/exercises/ex2/rdu-weather-history.csv")
# the infer schema option is the main working agent here.
# code that prints out the schema for weatherDataFrame
print(*weatherDataFrame, sep="\n")
...
windspeed = weatherDataFrame.select("windspeed").collect()
print(windspeed)

# COMMAND ----------

minTemp: float = weatherDataFrame.select(functions.min("temperaturemin")).collect()[0][0]
maxTemp: float = weatherDataFrame.select(functions.max("temperaturemax")).collect()[0][0]
...
weatherDataFrameWithYear: DataFrame = weatherDataFrame.withColumn("year", functions.year(functions.col("date").cast("date")))
...
task6DF: DataFrame = weatherDataFrameWithYear.groupBy("year").agg(
    ...
    functions.count("year").alias("num_entries"),
    functions.round(functions.avg("avgwindspeed"),2).alias("avg_wind_speed"))
task6DF.show()
...
min2012: float = task6DF.filter(task6DF.year == 2012).agg(functions.min("min_temperature")).collect()[0][0]
max2016: float = task6DF.filter(task6DF.year == 2016).agg(functions.max("max_temperature")).collect()[0][0]
entries2018: int = task6DF.filter(task6DF.year == 2018).agg(functions.count("num_entries")).collect()[0][0]
wind2008: float = task6DF.filter(task6DF.year == 2008).agg(functions.round(functions.avg("avg_wind_speed"), 2)).collect()[0][0]
print(entries2018)
...
fog_and_heavyfog_count_per_year = weatherDataFrameWithYear.groupBy("year").agg(
    functions.count("fog").alias("num_fog_days"),
    functions.count("fogheavy").alias("num_heavyfog_days"))
fog_and_heavyfog_count_per_year.show()
#yearWithMostDaysWithFog: int = ???
